...
1. **Cloud Shell** 명령 프롬프트에서 다음 명령을 입력하고 **Enter**를 눌러 배포에 사용할 Azure 지역을 값으로 지정하는 변수를 만듭니다. (자리 표시자 `<Azure region>`를 이 랩에서 리소스를 배포하려는 Azure 지역의 이름으로 바꿉니다. `az account list-locations` 명령은 구독에서 사용할 수 있는 모든 위치를 나열합니다.)
...
1. **aad0402-akscluster** 항목에 대한 **EXTERNAL-IP** 열의 값이 **\<보류 중\>** 에서 공용 IP 주소로 변경될 때까지 기다린 후 **Ctrl-C** 키 조합을 누릅니다. **aad0402-akscluster** 에 대한 **EXTERNAL-IP** 열의 공용 IP 주소를 기록합니다. 
...
1. **azure-vote-front** 항목에 대한 **EXTERNAL-IP** 열의 값이 **\<보류 중\>** 에서 공용 IP 주소로 변경될 때까지 기다린 후 **Ctrl-C** 키 조합을 누릅니다. **azure-vote-front** 에 대한 **EXTERNAL-IP** 열의 공용 IP 주소를 기록합니다. 
...
> **참고**: 이 솔루션은 https://docs.microsoft.com/ko-kr/azure/architecture/example-scenario/apps/devops-with-aks 페이지에 설명된 DevOps with Containers 솔루션에 기반합니다.
...
1. **AADesignLab0403-RG** 리소스 그룹 블레이드에서 리소스 목록을 검토하고 https://docs.microsoft.com/ko-kr/azure/architecture/example-scenario/apps/devops-with-aks 페이지에서 사용할 수 있는 정보와 비교합니다.

> **복습**: 이 연습에서는 AKS 아키텍처를 사용하여 DevOps를 배포했습니다.
